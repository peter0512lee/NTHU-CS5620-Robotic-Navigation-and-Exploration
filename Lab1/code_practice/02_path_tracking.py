...
if __name__ == "__main__":
    ...
    parser.add_argument("-s", "--simulator", type=str,
                        default="diff_drive", help="basic/diff_drive/bicycle")
    ...
    while(True):
        ...
        # Control
        end_dist = np.hypot(path[-1, 0]-simulator.state.x,
                            path[-1, 1]-simulator.state.y)
        if args.simulator == "basic":
            if end_dist > 10:
                next_v = 20
            else:
            ...
            next_lw = np.rad2deg(
                (next_v - np.deg2rad(next_w) * simulator.l / 2) / (simulator.wu / 2))
            next_rw = np.rad2deg(
                (next_v + np.deg2rad(next_w) * simulator.l / 2) / (simulator.wu / 2))
