...
def compute_gae(rewards, values, dones, last_values, last_dones, gamma=0.99, lamb=0.95):
    ...
    for t in reversed(range(n_step)):
        ...
        advs[t] = last_gae_lam = delta + gamma * \
            lamb*next_nonterminal*last_gae_lam
    ...
    def run(self, policy_net, value_net):
        ...
        for step in range(self.n_step):
            ...
            state_tensor = torch.tensor(np.expand_dims(
                self.states, axis=0), dtype=torch.float32, device=self.device)
            # state_tensor = torch.from_numpy(self.states).float()
            ...
            self.states, rewards, self.dones, info = self.env.step(
                self.mb_actions[step, :])

            self.mb_rewards[step, :] = rewards

        last_values = value_net(torch.from_numpy(
            self.states).float().to(self.device)).cpu().numpy()
        self.record()

        # 2. Compute returns
        # -------------------------------------
        mb_returns = compute_gae(self.mb_rewards, self.mb_values,
                                 self.mb_dones, last_values, self.dones, self.gamma, self.lamb)
